 ...
 */
    ...
    */
    computeAABB(){
        ...
        for (var i = 0; i < this.numVertices; i += 3) {
            minX = Math.min(minX, this.vBuffer[i]);
            minY = Math.min(minY, this.vBuffer[i + 1]);
            minZ = Math.min(minZ, this.vBuffer[i + 2]);
            
            maxX = Math.max(maxX, this.vBuffer[i]);
            maxY = Math.max(maxY, this.vBuffer[i + 1]);
            maxZ = Math.max(maxZ, this.vBuffer[i + 2]);
        }
        
        this.minXYZ = [minX, minY, minZ];
        this.maxXYZ = [maxX, maxY, maxZ];
    }
    ...
    loadFromOBJ(fileText)
    {
        ...
        for (var i = 0; i < lines.length; i++) {
            ...
            if (line[0] == "v") {
                ...
                this.fBuffer.push(parseInt(line[4]) - 1);
            }
        }
    ...
    */
    ...
    drawTrianglesReflect(shaderProgram){
        gl.bindBuffer(gl.ARRAY_BUFFER, this.VertexPositionBuffer);
        gl.vertexAttribPointer(shaderProgram.vertexPositionAttribute, this.VertexPositionBuffer.itemSize, 
                         gl.FLOAT, false, 0, 0);
